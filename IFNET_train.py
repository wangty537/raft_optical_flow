...
from simple_flow_net import create_simple_flow_loss
from IFNet_laploss import LapLoss, laploss
...
class SimpleFlowTrainer:
    ...
    def __init__(self, args):
        ...
        # ).to(self.device)
        self.model = IFNet_m_flow().to(self.device)
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 前向传播 - 训练时需要gt作为第三个输入
            # 注意：这里应该使用真实的中间帧作为gt，但当前数据集可能没有中间帧
            # 暂时使用img2作为占位符，实际应用中需要真实的中间帧标签
            input = torch.concat([img1, img2], dim=1) # b6hw (img0+img1)
            flow_list, mask_list, warped_list = self.model(input)

            #flow_preds = [flow[:,:2] for flow in flow_list]
            flow_preds = [flow[:,2:4] for flow in flow_list]
            # 保存光流图像
            if batch_idx == 1:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(self.args.log_dir, f'flow_simple_visualization_epoch_{epoch+1}_batch_{batch_idx}.png'), dpi=150, bbox_inches='tight')
                plt.close()  # 关闭图像以释放内存

            # 计算损失
           
            if self.args.unsuper:
                total_loss, loss_dict = laploss(warped_list, img1, img2, max_levels=5, channels=3, device=self.device)
            else:
    ...
    def validate(self, val_loader, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                # 前向传播
                input = torch.concat([img1, img2], dim=1) # b6hw (验证时不需要gt)
                flow_list, mask_list, warped_list = self.model(input)
                flow_preds = [flow[:,2:4] for flow in flow_list]
    ...
    def train(self):
        ...
        if self.args.resume:
            ...
            print(f"加载预训练模型: {self.args.pretrained}")
...
def main():
    ...
    # 损失函数参数
    parser.add_argument('--loss_weights', type=float, nargs='+', default=[0.32, 0.32, 0.32], 
                       help='多尺度损失权重')
    parser.add_argument('--smooth_weight', type=float, default=0.01, help='平滑性损失权重')
    parser.add_argument('--edge_weight', type=float, default=0.01, help='边缘感知损失权重')
    ...
    # 系统参数
    parser.add_argument('--gpu', type=int, default=1, help='GPU设备')
    parser.add_argument('--num_workers', type=int, default=4, help='数据加载线程数')
    
    # 保存参数
    parser.add_argument('--log_dir', type=str, default='./logs_IFNET_flow', help='日志目录')
    parser.add_argument('--save_dir', type=str, default='./logs_IFNET_flow/checkpoint', help='保存目录')
    parser.add_argument('--resume', action='store_true', help='恢复训练')

    parser.add_argument('--pretrained', default=None, help='恢复训练')
    parser.add_argument('--unsuper', default=False, help='恢复训练')
